...
func press(s *discordgo.Session, i *discordgo.InteractionCreate, button ButtonType) {
	...
	// Acknowledge interaction *immediately* — no files, no embeds
	err := s.InteractionRespond(i.Interaction, &discordgo.InteractionResponse{
		Type: discordgo.InteractionResponseDeferredMessageUpdate, // ← edits original message
		Data: &discordgo.InteractionResponseData{
		},
	})

	if err != nil {
		log.Printf("Failed to defer button response: %v", err)
		return
	}
	...
	pressDuration := 50 * time.Millisecond // e.g. 0.05s
        time.Sleep(pressDuration) // REQUIRED for proper input
	...
	if i.Member != nil && i.Member.User != nil {
		...
		found := false
		for j := range leaderboard.Entries {
			if leaderboard.Entries[j].Name == username {
				leaderboard.Entries[j].Keystrokes++
				found = true
				break
			}
		}
